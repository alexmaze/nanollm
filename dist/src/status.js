...
export class StatusStore {
    ...
    getBucket(modelName, timestamp) {
        const bucketStart = floorToFiveMinutes(timestamp);
        const buckets = this.modelBuckets.get(modelName) ?? new Map();
    ...
    recordAttempt(modelName, timestamp = Date.now()) {
        this.getBucket(modelName, timestamp).totalRequests += 1;
    }
    recordSuccess(modelName, durationMs, ttfbMs, usage, timestamp = Date.now(), streamDurationMs) {
    ...
    recordFailure(modelName, durationMs, timestamp = Date.now()) {
    ...
    getModelSeries(modelName, now = Date.now()) {
...
export function getHealthTone(successRate, totalRequests) {
    if (totalRequests === 0)
        return "empty";
    if (successRate >= 100)
        return "green";
...
export function formatBucketLabel(bucketStart) {
...
function buildStatusCell(bucketStart, metrics) {
    const successRate = metrics.totalRequests === 0 ? 0 : (metrics.successRequests / metrics.totalRequests) * 100;
    let avgTokenSpeed = null;
    if (metrics.totalStreamMs > 0 && metrics.outputTokens > 0) {
        avgTokenSpeed = metrics.outputTokens / (metrics.totalStreamMs / 1000);
...
export class SqliteStatusStore {
    ...
    addMetrics(modelName, timestamp, delta) {
        const bucketStart = floorToFiveMinutes(timestamp);
        this.db.prepare(`
    ...
    `).run(modelName, bucketStart, delta.totalRequests ?? 0, delta.successRequests ?? 0, delta.totalTtfbMs ?? 0, delta.ttfbSamples ?? 0, delta.totalDurationMs ?? 0, delta.durationSamples ?? 0, delta.totalStreamMs ?? 0, delta.streamSamples ?? 0, delta.nonCacheInputTokens ?? 0, delta.cacheReadInputTokens ?? 0, delta.outputTokens ?? 0);
        this.pruneOldBuckets();
    }
    recordAttempt(modelName, timestamp = Date.now()) {
    ...
    `).all(modelName, firstBucket, bucketStarts.at(-1) ?? firstBucket);
    ...
    hasBucket(modelName, bucketStart) {
