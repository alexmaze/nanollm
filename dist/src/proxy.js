import { SSEParser } from "./converters/streams.js";
import { denormalizeToOpenAIChatRequest, denormalizeToOpenAIResponsesRequest, denormalizeToAnthropicRequest, } from "./converters/requests.js";
import { normalizeOpenAIChatResponse, normalizeOpenAIResponsesResponse, normalizeAnthropicResponse, } from "./converters/responses.js";
import { normalizeUsage } from "./converters/shared.js";
import { ensureRecordedAttempt, setRecordedAttemptError, setRecordedAttemptResponseBody, setRecordedAttemptResponseMeta, } from "./record.js";
import { runInNewContext } from "node:vm";
import { ProxyAgent, fetch as undiciFetch } from "undici";
// ─── Upstream URL ───────────────────────────────────────────────────────────
export function getUpstreamURL(config) {
    return getUpstreamURLForPath(config);
}
export function getUpstreamURLForPath(config, imageOperation) {
...
// ─── Denormalize Request ────────────────────────────────────────────────────
function denormalizeRequest(config, normalized) {
    switch (config.provider) {
        case "openai-chat":
...
function applyOpenAIDefaults(provider, body) {
...
function applyModelBodyOverrides(config, body) {
    if (!config.body)
        return body;
    return deepMerge(body, config.body);
}
function applyModelBodyExpression(config, body) {
    if (!config.bodyExpression)
        return body;
    let result;
    try {
...
function applyModelBodyTransforms(config, body) {
...
function stripOpenAIResponsesUnstoredItemIds(config, body) {
    if (config.provider !== "openai-responses" || !isPlainObject(body) || body.store !== false || !Array.isArray(body.input))
        return body;
    let changed = false;
    const input = body.input.map((item) => {
        if (!isPlainObject(item) || typeof item.type !== "string" || !OPENAI_RESPONSES_UNSTORED_ITEM_ID_TYPES.has(item.type) || !("id" in item))
            return item;
        changed = true;
...
function preparePassthroughBody(config, rawBody, stream) {
    return stripOpenAIResponsesUnstoredItemIds(config, applyModelBodyTransforms(config, { ...rawBody, model: config.model, stream }));
}
function isJsonContentType(headers) {
    return headers.get("content-type")?.toLowerCase().includes("application/json") ?? false;
}
function isMultipartContentType(headers) {
    return headers.get("content-type")?.toLowerCase().includes("multipart/form-data") ?? false;
}
function prepareRawJsonBody(config, body) {
...
function replaceRecordedRequestModel(recordedRequestBody, model) {
...
function prepareRawMultipartBody(config, body, incomingHeaders, recordedRequestBody) {
    if (typeof body !== "string" && !(body instanceof Uint8Array))
        return undefined;
...
async function prepareRawBody(config, body, incomingHeaders, recordedRequestBody) {
...
// ─── Normalize Response ─────────────────────────────────────────────────────
function normalizeUpstreamResponse(provider, body) {
    switch (provider) {
        case "openai-chat":
...
// ─── Shared fetch ───────────────────────────────────────────────────────────
function getForwardHeaders(config, options) {
    return {
...
async function upstreamFetch(config, body, stream, options) {
    return upstreamFetchToUrl(config, getUpstreamURL(config), body, stream, getForwardHeaders(config, options), options);
}
async function upstreamFetchToUrl(config, url, body, stream, headers, options, recordedRequestBody = typeof body === "string" ? body : "[binary body]") {
    ...
    try {
        ...
        if (abortController?.signal.aborted && error === abortController.signal.reason) {
            ...
            const err = new Error(`Upstream TTFB timeout after ${timeoutMs}ms`);
            err.cause = error;
            throw err;
        }
        setRecordedAttemptError({
    ...
    if (contentType.includes("text/html")) {
        ...
        const err = new Error("Upstream returned HTML response (possible error page)");
        err.status = 200;
        err.upstream = text;
        throw err;
    }
    if (stream && !contentType.includes("text/event-stream")) {
        ...
        const err = new Error(`Upstream returned non-SSE Content-Type for stream request: ${contentType}`);
        err.status = 200;
        err.upstream = text;
        throw err;
    }
...
function reconstructStream(bufferedChunks, reader) {
    let index = 0;
...
async function validateStreamContent(body, options) {
    ...
    try {
        while (true) {
            ...
            if (done) {
                ...
                const err = new Error("Upstream SSE stream ended with no real content (ping-only or empty)");
                err.status = 200;
            ...
            if (totalBytes >= MAX_VALIDATION_BUFFER_BYTES) {
                ...
                const err = new Error(`Upstream SSE stream exceeded ${MAX_VALIDATION_BUFFER_BYTES} bytes with no real content`);
                err.status = 200;
                err.upstream = bufferedText;
                reader.cancel().catch(() => { });
                throw err;
        ...
        if (error instanceof Error && "upstream" in error)
            throw error;
        throw error;
    }
}
function getRawForwardHeaders(config, incomingHeaders, options) {
    const headers = {};
...
export async function passthroughRawRequest(config, body, incomingHeaders, options) {
    ...
    const { response, timing } = await upstreamFetchToUrl(config, url, upstreamBody, false, headers, options, recordedRequestBody);
...
export async function passthroughRequest(config, rawBody, options) {
...
export async function passthroughStreamRequest(config, rawBody, options) {
    ...
    if (!response.body)
        throw new Error("Upstream returned no streaming body");
...
export async function forwardRequest(config, normalized, options) {
...
export async function forwardStreamRequest(config, normalized, options) {
...
function buildTestRequestBody(config) {
    const base = config.base_url.replace(/\/+$/, "");
    switch (config.provider) {
        case "openai-chat":
            return {
...
export async function testUpstream(config) {
