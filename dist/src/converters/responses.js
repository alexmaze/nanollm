import { collapseText, denormalizeUsageToAnthropic, denormalizeUsageToOpenAIChat, denormalizeUsageToOpenAIResponses, fail, normalizeUsage, parseJson, qualifyOpenAIResponsesToolName, refusal, splitQualifiedOpenAIResponsesToolName, text, unwrapResponsesCustomToolInput, wrapResponsesCustomToolInput, } from "./shared.js";
...
export function normalizeOpenAIChatResponse(response) {
    ...
    return {
        ...
        message: {
            ...
            toolCalls: message?.tool_calls?.map((toolCall) => toolCall.type === "function"
                ? { kind: "function", id: toolCall.id, name: toolCall.function.name, payload: toolCall.function.arguments }
                : { kind: "custom", id: toolCall.id, name: toolCall.custom.name, payload: toolCall.custom.input }) ?? [],
        },
        usage: normalizeUsage(response.usage),
    };
}
export function normalizeOpenAIResponsesResponse(response) {
    const parts = [];
    const toolCalls = [];
...
function normalizeOpenAIChatThinkingParts(message) {
...
export function denormalizeToOpenAIChatResponse(response) {
    ...
    const toolCalls = response.message.toolCalls?.map((toolCall) => toolCall.kind === "function"
    ...
    return {
        ...
        usage: denormalizeUsageToOpenAIChat(response.usage),
    };
}
export function denormalizeToOpenAIResponsesResponse(response) {
    ...
    return {
        ...
        model: response.model,
        output_text: collapseText(visibleParts),
        error: null,
                ...
                ? [
                    {
                        ...
                        role: "assistant",
                        status: "completed",
                        content: visibleParts.map((part) => part.type === "text"
        ...
        usage: denormalizeUsageToOpenAIResponses(response.usage),
    };
}
export function denormalizeToAnthropicResponse(response) {
    return {
        id: response.id,
        type: "message",
        role: "assistant",
        ...
        content: [
            ...
            ...(response.message.toolCalls?.map((toolCall) => {
                if (toolCall.kind !== "function")
                    fail("Anthropic response conversion only supports function-style tool calls");
                return {
                    type: "tool_use",
                    id: toolCall.id,
                    caller: { type: "direct" },
        ...
        usage: denormalizeUsageToAnthropic(response.usage) ?? {
            input_tokens: 0,
            output_tokens: 0,
...
function normalizeChatFinishReason(reason) {
