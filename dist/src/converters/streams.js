...
export class SSEParser {
    ...
    hasBufferedRealData() {
        ...
        for (const line of pending.split(/\r?\n/)) {
            ...
            if (!data || "[DONE]".startsWith(data) || data === "[DONE]")
                continue;
            return true;
        }
        return false;
...
export class OpenAIChatStreamParser {
    ...
    ended = false;
    textBlockIndex = null;
    refusalBlockIndex = null;
    thinkingBlockIndex = null;
    toolCallMap = new Map();
    openContentBlocks = new Set();
    ...
    emitEnd(out, usage) {
        ...
        out.push({
            type: "end",
            finishReason: this.pendingFinishReason === "stop" && this.toolCallMap.size > 0
                ? "tool_calls"
    ...
    parse(event) {
        ...
        switch (event.type) {
            ...
            case "response.completed": {
                ...
                const hasToolCalls = this.seenToolCall || r.output?.some((item) => item.type === "function_call" || item.type === "custom_tool_call");
                const finishReason = r.status === "incomplete" ? "length" : hasToolCalls ? "tool_calls" : "stop";
                out.push({ type: "end", finishReason, usage: normalizeUsage(r.usage) });
                break;
            }
...
// ─── OpenAI Chat Emitter ────────────────────────────────────────────────────
export class OpenAIChatStreamEmitter {
    id = "";
    model = "";
    created = 0;
    blockTypes = new Map();
...
export class ResponsesStreamEmitter {
    ...
    blockToMapping = new Map();
    blockTypes = new Map();
    accumulated = new Map();
    toolCallInfo = new Map();
    wrappedToolDecodedLengths = new Map();
    itemIds = new Map();
    ...
    makeItemId(outputIndex, prefix) {
        const id = `${prefix}_${outputIndex}`;
        this.itemIds.set(outputIndex, id);
        return id;
    }
    buildResponseShell(status, usage) {
        return {
            id: this.id,
            object: "response",
...
function decodeWrappedCustomToolInputPrefix(argumentsText) {
...
function findWrappedCustomToolStringStart(argumentsText) {
    const match = /^\s*\{\s*"(?:content|arg)"\s*:\s*"/.exec(argumentsText);
    return match ? match[0].length : -1;
}
function decodeJsonStringPrefix(text) {
    let result = "";
...
function mapJsonEscapeCharacter(ch) {
    switch (ch) {
        case "\"":
            return "\"";
        case "\\":
...
export class AnthropicStreamEmitter {
    ...
    blockTypes = new Map();
    blockIndex = 0;
    normalizedToAnthropicIndex = new Map();
    toolKinds = new Map();
    encodeCustomToolDelta(delta) {
        return JSON.stringify(delta).slice(1, -1);
    }
    getBlockIndex(normalizedIndex) {
 ...
 * ```
 */
export function createSSETransformStream(from, to) {
...
// ─── Factory ────────────────────────────────────────────────────────────────
export function createStreamConverter(from, to) {
    return new StreamConverter(from, to);
}
export function createSSEConverter(from, to) {
    return new SSEStreamConverter(from, to);
}
export function createUsageCollector(format) {
